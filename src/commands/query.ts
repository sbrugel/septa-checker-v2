...
export default class extends Command {
    ...
    async run(interaction: CommandInteraction): Promise<void> {
        ...
        .then((res) => {
            ...
            return interaction.reply({ content: `Train no. **${train.trainno}** going to ${train.dest} is currently running **${train.late > 0 ? `${train.late} minute${train.late !== 1 ? `s` : ``} late` : `on time`}** - formed of ${BOT.verboseConsists ? train.consist : `${train.consist.split(',').length} cars`}. Last seen at ${train.currentstop}.`, ephemeral: true });
        });
    }
}